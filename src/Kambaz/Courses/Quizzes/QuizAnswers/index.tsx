...
export default function QuizAnswers() {
  ...
  return (
    <Tabs
      ...
      variant="tabs"
    >
      <Tab eventKey="preview" title={<span className={`${key === 'preview' ? 'text-body' : 'text-danger'}`}>{`${currentUser.role === "FACULTY" ? "Preview" : "Current Attempt"}`}</span>}>
      ...
      <Tab eventKey="previousAttempt" title={<span className={`${key === 'preview' ? 'text-danger' : 'text-body'}`}>Previous Attempt</span>}>
